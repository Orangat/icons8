<template lang="pug">
  div
    app-modal(
      ...
      // effects preview
      .effect-modal-preview
        canvas.effect-modal-preview-canvas.transparent-background(width="252" height="252" hidpi="off")

        .effect-modal-preview-footer
          .effect-modal-preview-icon.transparent-background(v-html="svgPreview")
...
<script>
  ...
  export default {
    ...
    methods: {
      initEditor (effect) {
        this.canvas = this.$el.querySelector('.effect-modal-preview-canvas')
      ...
      toggleEffect (effect) {
        console.log('effect.isActive', effect.isActive)
        if (effect.isActive) {
          effectsService.enableEffect(effect.id)
        } else {
...
<style lang="scss">
  ...
  .effects-modal {
    ...
    .effect-modal-preview-text {
      margin-bottom: 4px;
      display: inline-block;
      font-size: 14px;
      color: $color-font-dark;
    }
    .effect-modal-preview-header {
